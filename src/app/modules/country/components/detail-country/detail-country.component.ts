...
export class DetailCountryComponent extends BaseComponent implements OnInit{
  ...
  async getAreaByCode() {
    this.showSpinner();
    var result = await this.countryService.getCountryByCode(this.code, () => this.hideSpinner());
    this.countryForm.patchValue({
      ...
      europeanCommunityMember: result?.europeanCommunityMember,
      euroCurrency: result?.euroCurrency,
      iban: result?.iban,
      sepa: result?.sepa,
      controlBankCheckType:result?.controlBankCheckType,
      isoCode: result?.isoCode,
      internationalNumericCode: result?.internationalNumericCode,
      taxRegistrationNumberLength: result?.taxRegistrationNumberLength,
      taxStampExcluded: result?.taxStampExcluded,
      firstAddressLine: result?.firstAddressLine,
      secondAddressLine: result?.secondAddressLine,
      thirdAddressLine: result?.thirdAddressLine,
      fourthAddressLine: result?.fourthAddressLine,
      fifthAddressLine: result?.fifthAddressLine,
      postalCode: result?.postalCode,
      town: result?.town,
      district: result?.district,
      validateIban: result?.validateIban,
    })
    this.breadcrumbService.setItems([
      ...
      {
        label: this.countryForm?.value?.shortText,
        routerLink: ['/country-list/' + this.code]
      }
    ])
  }
  ...
  save(value, close: boolean) {
    ...
    this.confirmationService.confirm({
      ...
      accept: async () => {
        ...
        close ? this.router.navigate(['/country-list/']) : this.getAreaByCode();
